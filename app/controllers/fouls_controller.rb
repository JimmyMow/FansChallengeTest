...
def index
  ...
  def create
    ...
    foul = params[:foul].downcase
    foul = Foul.where("commit_player_id = ? AND fouled_player_id = ? AND quarter = ? AND ref_id = ? AND game_id = ? AND name = ? AND time = ?", commit_player, fouled_player, params[:quarter], ref, params[:game_id], foul, params[:time]).first
